...
eval_batch_size = 32  # number of chunks to batch together
...
# Pre-tokenize all paragraphs and collect (input, target) chunks
all_chunks = []  # list of (inp_ids, tgt_ids)
...
with torch.no_grad():
    with ctx:
        for i in range(0, len(all_chunks), eval_batch_size):
            batch = all_chunks[i:i + eval_batch_size]
            # Pad to the longest chunk in this batch
            max_len = max(len(tgt) for _, tgt in batch)
            x_batch = torch.zeros(len(batch), max_len, dtype=torch.long, device=device)
            y_batch = torch.full((len(batch), max_len), -1, dtype=torch.long, device=device)
            for j, (inp, tgt) in enumerate(batch):
